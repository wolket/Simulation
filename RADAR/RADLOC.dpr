...
begin
  ...
  for I := 0 to tar-1 do begin
    init_ttype := random(2);                                              //определение цели(случайное - самолёт/ракета)
    ...
    //ракета
      ...
      write('Input missle acceleration: '); readln(init_acc);             //нужен ещё 1 парамет - ускорение
...
begin
  ...
  write('Input output file name: '); readln(filename);                  //считывание имени файла
  assignfile(f, filename); rewrite(f);                                  //открытие на запись
  writeln(f, ' Dist  ',';  Az   ',';num',';type');                      //шапка для файла
  radar := RLC.create(t0);                                              //создание РЛС
